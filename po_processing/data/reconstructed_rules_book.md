...
- `has_po`: Whether the document contains an invoice
- `has_waf`: Whether the document contains a work authorization form
- `po_count`: Number of invoices detected
...
|-------|------|----------|-------|
| `po_number` | String | Yes | Unique invoice identifier |
| `po_date` | Date | Yes | Format: YYYY-MM-DD |
| `po_total_inc_tax` | Decimal | Yes | Total including tax |
| `po_total_ex_tax` | Decimal | Yes | Total excluding tax |
...
- An empty invoice structure is created with `po_number = "EXTRACTION_FAILED"`
...
```
Expected: po_total_inc_tax = po_total_ex_tax + tax_amount
...
```
Expected: sum(line_items.amount_ex_tax) = po_total_ex_tax
