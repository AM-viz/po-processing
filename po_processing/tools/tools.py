"""
Function tools for the PO Processing unified agent.
...
# Ensure po_processing package and shared_libraries are importable
AGENT_ROOT = AGENT_PKG_DIR.parent  # po_processing/ (outer)
...
def load_case(case_id: str) -> dict:
    ...
    inv_processing = pp.get("PO Processing", {})

    return {
        "case_id": case_data.case_id,
        "summary": case_data.summary,
        "decision": inv_processing.get("PO Status", "UNKNOWN"),
...
def build_rule_discovery_context(case_id: str, sme_feedback: str) -> dict:
    ...
    agent_decision = inv_processing.get("PO Status", "UNKNOWN")
    rejection_reason = inv_processing.get("Rejection Reason", "")
    ...
    invoice_json = json.dumps(
        case_data.context.get("po", {}), indent=2, default=str
    )[:3000]
