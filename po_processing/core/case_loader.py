...
class CaseLoaderAgent:
    ...
    def _build_summary_decision(self, data: CaseData, lines: list) -> str:
        ...
        inv_status = pp.get("PO Processing", {}).get(
            "PO Status", "Unknown"
        )
    ...
    def _build_summary_key_details(self, ctx: dict, lines: list) -> None:
        ...
        inv_total = invoice.get("po_total_inc_tax", "?")
        currency = invoice.get("currency", "")
        lines.append(f"  PO Total: ${inv_total} {currency}".rstrip())

        inv_date = invoice.get("po_date", "?")
        lines.append(f"  PO Date: {inv_date}")
