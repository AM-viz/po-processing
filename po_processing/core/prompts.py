...
Your job: Given an purchase order processing case where the acting agent produced an \
...
any_item_contains    - Checks if ANY item in an array of objects has a string field containing the pattern.
                       ...
                       CORRECT:   "field": "extraction.po.line_items",  "operator": "any_item_contains", "value": "portapack|oxygen"
                       WRONG:     "field": "purchase order.line_items[*].description",  (this path does NOT work)
...
- For line item matching, use field "extraction.po.line_items" with "any_item_contains".
...
PO data (from extraction):
  purchase order.vendor_name            - Vendor name from purchase order
  purchase order.customer_name          - Customer name from purchase order
  purchase order.vendor_tax_id          - Vendor tax ID (e.g., ABN)
  purchase order.po_total_inc_tax  - Total including tax
  purchase order.po_total_ex_tax   - Total excluding tax
  purchase order.tax_amount             - Tax amount
  purchase order.currency               - Currency code (AUD, USD, EUR, etc.)
  purchase order.po_date           - PO date (YYYY-MM-DD)
  purchase order.line_items             - Array of line item objects (use with any_item_contains)
  purchase order._tax_id_validation.valid  - Boolean: tax ID checksum result

Extraction (raw artifact):
  extraction.po.vendor_tax_id  - Tax ID from extraction
  extraction.po.line_items     - Array of line item objects (use with any_item_contains)
...
Example 1 - Customer entity variant rejection:
  ...
  {"field": "purchase order.customer_name", "operator": "regex_match", "value": "(?i)\\bacme\\s*(pty|group|holdings)\\b"}
...
Example 2 - WAF hours enforcement (agent accepted but should reject):
  ...
  {"field": "extraction.po.line_items", "operator": "any_item_contains", "value": "labour|labor|technician|installation"}

Example 3 - Tax ID validation inconsistency:
  {"field": "decision_phase1", "operator": "equals", "value": "CONTINUE"}
  {"field": "purchase order._tax_id_validation.valid", "operator": "is_false", "value": null}
