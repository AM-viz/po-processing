#!/usr/bin/env python3
"""
General PO Processing Multi-Agent System
...
Usage:
    python general_po_agent.py --base-dir path/to/invoices
    python general_po_agent.py --case path/to/single/case
    python general_po_agent.py --base-dir path/to/invoices --config config.json
...
try:
    ...
    from po_processing.core.llm_client import (
        GenerationConfig,
        GenerativeModel,
        Part,
    )
...
SCRIPT_DIR = Path(__file__).resolve().parent  # used to resolve relative --config paths
...
class PurchaseOrderExtraction(BaseModel):
    ...
    po_number: str | None = "UNKNOWN"
    po_date: str | None = ""
    po_total_inc_tax: float | None = 0.0
    po_total_ex_tax: float | None = 0.0
    tax_amount: float | None = 0.0
    vendor_tax_id: str | None = None
    vendor_name: str | None = "UNKNOWN"
    customer_name: str | None = None
    currency: str | None = "AUD"
    line_items: list[PurchaseOrderLineItem] | None = []
...
class ClassifierAgent(BaseAgent):
    ...
    def run(self, source_folder: Path, output_folder: Path) -> dict:
        ...
        files_info = {}
        summary = {"po_count": 0, "waf_count": 0}
        invoice_sources = []
        waf_sources = []
        ...
        for file_path in source_folder.iterdir():
            ...
            if file_path.suffix.lower() == ".pdf":
                ...
                if doc_info.has_po:
                    ...
                    summary["po_count"] += doc_info.po_count
        ...
        print(
            f"   Found {summary['po_count']} invoice(s), {summary['waf_count']} WAF(s)"
        )
    ...
    def run(
        ...
        if invoice_sources:
            ...
            try:
                ...
                print(
                    f"   Extracted invoice: {invoice_data.get('po_number', 'N/A')}"
                )
            except Exception as e:
                extraction_failed = True
                extraction_error = str(e)
                invoice_data = self._empty_po()
        else:
            extraction_failed = True
        ...
        output = self.create_output(
            {
                ...
                "po_count": classification.get("summary", {}).get(
                    "po_count", 0
                ),
...
Return ONLY valid JSON with these fields:
{
  "po_number": "string",
  "po_date": "YYYY-MM-DD",
  "po_total_inc_tax": decimal,
  "po_total_ex_tax": decimal,
  "tax_amount": decimal,
...
Extract ALL line items. Use null for missing optional fields."""
        ...
        result, _ = call_gemini_with_pdf(
            pdf_path, prompt, _gcp_config.HEAVY_MODEL, PurchaseOrderExtraction
        )
...
Return ONLY valid JSON:
{
  ...
  "date": "YYYY-MM-DD or null"
    ...
    def _empty_po(self) -> dict:
        """Return empty invoice structure"""
        return {
            "po_number": "EXTRACTION_FAILED",
            "po_date": "",
            "po_total_inc_tax": 0.0,
            "po_total_ex_tax": 0.0,
            "tax_amount": 0.0,
            "vendor_tax_id": "",
            "vendor_name": "UNKNOWN",
    ...
    def run(self, output_folder: Path, extraction: dict) -> dict:
        ...
        if extraction.get("extraction_failed"):
            validations.append(
                {
                    "step": "1.1",
                    "rule": "PO extraction must succeed",
                    "passed": False,
                    "evidence": extraction.get(
                        "extraction_error", "Extraction failed"
                    ),
                    "rejection_template": "Document is not a valid Tax PO",
                }
            )
        else:
            validations.append(
                {
                    ...
                    "rule": "PO extraction must succeed",
                    "passed": True,
                    "evidence": f"Extracted invoice {invoice.get('po_number')}",
                }
            )
        ...
        # Step 1.5: Single PO Check
        po_count = extraction.get("po_count", 1)
        validations.append(
            {
                "step": "1.5",
                "rule": "Single invoice per submission",
                "passed": po_count <= 1,
                "evidence": f"Found {po_count} invoice(s)",
                "rejection_template": None
                if po_count <= 1
...
class Phase2ValidatorAgent(BaseAgent):
    ...
    def run(self, output_folder: Path, extraction: dict, phase1: dict) -> dict:
        ...
        validations.append(
            {
                "step": "2.1",
                "rule": "PO has line items",
                "passed": line_count > 0,
                "evidence": f"Found {line_count} line item(s)",
                "rejection_template": None
                if line_count > 0
                else "PO charges not itemized",
            }
        )
...
class Phase3ValidatorAgent(BaseAgent):
    ...
    def run(self, output_folder: Path, extraction: dict, phase2: dict) -> dict:
        ...
        po_date = parse_date(invoice.get("po_date"))
        today = date.today()

        if po_date:
            is_future = po_date > today
            validations.append(
                {
                    "step": "3.3",
                    "rule": "PO not future dated",
                    "passed": not is_future,
                    "evidence": f"PO date: {invoice.get('po_date')}",
                    "rejection_template": None
                    if not is_future
                    else "PO is future dated",
                }
            )
...
class Phase4ValidatorAgent(BaseAgent):
    ...
    def run(self, output_folder: Path, extraction: dict, phase3: dict) -> dict:
        ...
        total_inc = invoice.get("po_total_inc_tax", 0)
        total_ex = invoice.get("po_total_ex_tax", 0)
        ...
        validations.append(
            {
                ...
                else "PO amounts do not reconcile",
            }
        )
            ...
            validations.append(
                {
                    ...
                    "passed": hours_valid,
                    "evidence": f"PO: {invoice_hours}h, Authorized: {waf_hours}h, Work type: {work_type}",
                    "rejection_template": None
                    if hours_valid
                    else "PO does not match work authorization",
                }
            )
            ...
            validations.append(
                {
                    ...
                    "passed": False,
                    "evidence": (
                        f"PO: {invoice_hours}h, Work type: {work_type} (non-exempt), "
                        f"WAF authorized hours: {waf_hours} — no authorization for billed labour"
                    ),
                    "rejection_template": "PO does not match work authorization",
                }
            )
...
class OutputGeneratorAgent(BaseAgent):
    ...
    def run(
        ...
        if decision == "ACCEPT":
            outcome = f"PO accepted for payment on {timestamp}"
        elif decision == "REJECT":
            reason = (
                rejection_template or rejection_reason or "Validation failed"
            )
            outcome = f"PO rejected on {timestamp}: {reason}"
        else:
            outcome = f"PO requires review as of {timestamp}"

        # Build output
        output_data = {
            "PO Processing": {
                "PO Type": "Normal",
                "PO Status": invoice_status,
                "PO Source": "Email",
            },
            "PO Details": {
                "PO Number": invoice.get("po_number", ""),
                "PO Date": invoice.get("po_date", ""),
                "PO Total": f"{invoice.get('po_total_inc_tax', 0):,.2f}",
                "Pretax Total": f"{invoice.get('po_total_ex_tax', 0):,.2f}",
...
def process_po(source_folder: Path) -> dict:
...
def main():
    parser = argparse.ArgumentParser(
        description="General PO Processing Agent"
    )
    parser.add_argument(
    ...
    print("=" * 60)
    print("GENERAL PO PROCESSING AGENT")
    print("=" * 60)
