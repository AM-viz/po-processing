...
Usage:
    ...
    schema = master.get_extraction_schema("po")
...
_DEFAULT_MASTER_DATA_PATH = _SCRIPT_DIR / "po_master_data.yaml"
...
class MasterData:
    ...
    def resolve_dotpath(self, data: dict[str, Any], path: str) -> Any:
        """Resolve a dot-separated path like 'PO Processing.PO Status'."""
        parts = path.split(".")
        current = data
        for part in parts:
...
def load_master_data(path: str | None = None) -> MasterData:
    ...
    Args:
        path: Path to master_data.yaml (or .json). If None, looks for
              the default po_master_data.yaml in the project root.
