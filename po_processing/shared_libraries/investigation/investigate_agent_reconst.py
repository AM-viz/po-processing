...
class ReconstructedRulesExtractor:
    ...
    - #### Step 15: Duplicate PO Detection
    """
    ...
    def get_duplicate_detection_rules(self) -> str:
        """Extract Step 15: Duplicate PO Detection (Phase 2.5)"""
        return self.extract_step("15")

    def get_invoice_type_rules(self) -> str:
        """Extract Section 9.3: PO Type Determination"""
...
class DataSourceValidator:
    ...
    _DEFAULT_INVOICE_CONTENT_FIELDS: ClassVar[dict[str, str]] = {
        "invoice_total_inc_gst": "PO total including GST",
        "invoice_total_ex_gst": "PO total excluding GST",
        "gst_amount": "GST/tax amount",
        "po_number": "PO number from PDF",
        "po_date": "PO date from PDF",
    ...
    def validate_balance_calculation(
        ...
        """
        invoice = extraction_data.get("po", {})
...
1. Work type is determined from PO LINE ITEMS using LLM classification
...
"""
    ...
    def validate_duplicate_detection(
        ...
        """
        Validate Step 15: Duplicate PO Detection per Section 5.5.
        """
...
2. Composite key: po_number + vendor_number
3. Decision logic:
   - No matches → CONTINUE
   - Match + SENT status → REJECT ("Document already processed")
   - Match + CANCEL status → REJECT ("PO is Cancelled")
...
- Work Type: {work_type}
- PO Labour Hours: {invoice_labour_hours}
...
- Calculate: Effective PO Hours = max(invoice_labour_hours, 1.0)
- Calculate: Threshold = waf_total_hours + investigator_tolerance
- If Effective PO Hours <= Threshold → Expected: Continue
- If Effective PO Hours > Threshold → Expected: Reject
...
**CASE DATA:**
- Has Subcontractor PO: {has_subcontractor}
...
{{
    ...
    "po_total_inc_tax": {invoice.get("po_total_inc_tax", 0)},
...
def _extract_final_decision(
    ...
    if not final_decision:
        po_processing = (
            postprocessing_data.get("PO Processing", {})
            if postprocessing_data
            else {}
        )
        final_status = po_processing.get("PO Status", "UNKNOWN")
...
def _extract_invoice_type(
    ...
    if not invoice_type and postprocessing_data:
        po_processing = postprocessing_data.get("PO Processing", {})
        invoice_type = po_processing.get("PO Type", "Unknown")
...
def extract_processing_summary(
    ...
    invoice_data = (
        extraction.get("po", {}) if "invoice" in extraction else extraction
    )
    ...
    if not invoice_data:
        ...
        required_fields = ["vendor_name", "po_number", "po_date"]
...
def _load_and_print_config(args) -> str:
    ...
    domain_name = _md.display_name if _md else "PO Processing"
    print("=" * 80)
