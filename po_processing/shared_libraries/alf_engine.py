...
from po_processing.core import storage  # noqa: E402

SCRIPT_DIR = Path(__file__).parent  # shared_libraries/
AGENT_PKG_DIR = storage.PACKAGE_DIR  # po_processing/ package root
...
# Bedrock LLM config (mirrors acting_agent settings)
def _get_llm_model():
    from po_processing.core.llm_client import get_sonnet_model_id  # noqa: PLC0415, I001
...
class ConditionEvaluator:
    ...
    def _resolve_dynamic_value(value: Any, data: dict) -> Any:
        ...
        Examples:
            ...
            "_DYNAMIC_extraction.po.abn_"    -> "11089895732"
...
class ActionExecutor:
    ...
    def _apply_override_decision(output: dict, value: Any) -> None:
        ...
        if value in status_map:
            ActionExecutor._set_field(
                output, "PO Processing.PO Status", status_map[value]
            )
    ...
    def _recalculate(output: dict, action: dict, context: dict):
        ...
        if formula == "sum_line_items_ex_gst":
            ...
            total = float(
                ConditionEvaluator.resolve_field(
                    output, "PO Details.PO Total"
                )
                or "0"
            )
            pretax = float(
                ConditionEvaluator.resolve_field(
                    output, "PO Details.Pretax total"
                )
                or "0"
            )
...
VALIDATION RULES TO APPLY (continue from where agent stopped):
  Step 2.1: PO charges must be itemized (line_items > 0)
  Step 3.2: Tax ID checksum validation (ABN mod 89)
  Step 3.3: PO date must not be in the future
...
5. If all remaining validations pass -> PO Status = "Pending Payment"
6. If any remaining validation fails -> PO Status = "Rejected" with template
...
3. If ALL remaining validations pass, set PO Status to "Pending Payment".
4. If any remaining validation FAILS, set PO Status to "Rejected" with the
...
{{
  "PO Processing.PO Status": "Rejected",
  "Line Items": [...]
}}
...
class LLMActionExecutor:
    ...
    def _get_model(cls):
        ...
        if cls._model is None:
            try:
                from po_processing.core.llm_client import (  # noqa: PLC0415
                    GenerationConfig,
                    GenerativeModel,
                )
    ...
    def execute(output: dict, action: dict, context: dict) -> dict:
        ...
        try:
            ...
            output.setdefault("PO Processing", {})["PO Status"] = (
                "To verify"
            )
            ...
            execution_log.append(
                {
                    ...
                    "before_status": before.get("PO Processing", {}).get(
                        "PO Status"
                    ),
                    "after_status": revised.get("PO Processing", {}).get(
                        "PO Status"
                    ),
                }
            )
...
def run_alf_on_case(
    ...
    context["decision"] = postprocessing.get("PO Processing", {}).get(
        "PO Status", ""
    )
